{ config, lib, ... }:
let
  cfg = config.joona.services.adguard;

  port = 3002; # 3000 is the hundred container, 3001 grafana
  domain = "dns.joona.codes";
in
{
  options.joona.services.adguard.enable = lib.mkEnableOption "AdGuard Home, the tailnet's resolver";
  ...
  config = lib.mkIf cfg.enable {
    ...
    joona.vhosts.${domain} = { inherit port; };
  };
}
