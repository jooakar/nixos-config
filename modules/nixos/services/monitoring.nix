...
let
  cfg = config.joona.services.monitoring;
...
in
{
  options.joona.services.monitoring.enable = lib.mkEnableOption "Prometheus, Loki, Alloy and Grafana";
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Single process, single replica, local disk
    services.loki = {
      enable = true;
      configuration = {
        auth_enabled = false;
    ...
    joona.vhosts."grafana.joona.codes".port = grafanaPort;
  };
}
