...
in
{
  ...
  options.joona.tailnet.cidrs = lib.mkOption {
    ...
    description = ''
      What the tailnet sources traffic from, and the whole of what a non-public
      vhost lets past.
    '';
  };
  ...
  config = {
    ...
    services.fail2ban = {
      enable = true;
      ignoreIP = [ "127.0.0.0/8" ] ++ config.joona.tailnet.cidrs;
    };
